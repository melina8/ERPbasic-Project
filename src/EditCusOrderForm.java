...
public class EditCusOrderForm {
    ...
    ObservableList<OrdItems>  cusItemsList = FXCollections.observableArrayList();
    ...
    TreeMap<String,Integer> mapStorage = new TreeMap<>();
    TreeMap<String,Integer> mapOrdered = new TreeMap<>();
    TreeMap<String,Integer> mapTableView = new TreeMap<>();
    TreeMap<String, Integer> mapSubtraction = new TreeMap<>();

    TreeMap<String,Integer> mapOrderedRemove = new TreeMap<>();
    TreeMap<String,Integer> mapTableViewRemove = new TreeMap<>();


    Connection connection;


    public EditCusOrderForm(Connection conn, String state, int ship_no, LocalDate ship_date, String cusId, String customerName) {
        connection = conn;

        CustOrder cor = new CustOrder(ship_no, cusId, customerName, ship_date);
        ...
        itemsColumn.setOnEditCommit(new EventHandler<TableColumn.CellEditEvent<OrdItems, Integer>>() {
            ...
            public void handle(TableColumn.CellEditEvent<OrdItems, Integer> event) {
                ...
                Double newFinalCost2 = ((uCost+pTax)-((uCost+pTax)*pDiscount)/100)*newItems;
        ...
        discountColumn.setOnEditCommit(new EventHandler<TableColumn.CellEditEvent<OrdItems, Integer>>() {
            ...
            public void handle(TableColumn.CellEditEvent<OrdItems, Integer> event){
                ...
                Double uCost = orditem.getUnitCost();
                Double pTax = orditem.getProductTax();
                Double newFinalCost2 = ((uCost+pTax)-((uCost+pTax)*newDiscount)/100)*pItems;
                DecimalFormat df = new DecimalFormat("#.00");
                Double newFinalCost = Double.parseDouble(df.format(newFinalCost2));
        ...
        checkState(state, cor);
        loadForm(cor);
        loadTableView(ship_no);
        ...
        btnUpdate.setOnAction(e-> {
            ...
            if (response) {
                ...
                //everything is ok! add data to DB
                    ...
                    //1st step: delete previous data from the DB (tables: storage_area, ordered_items, shipping_orders )
                    ...
                    //delete previous data from ordered_items table
                    try {
                    ...
                    //delete previous data from shipping_orders table
                    try {
                    ...
                    //2nd step: update tables with the new data (shipping_orders, ordered_items, storage_area)
                    //update shipping_orders table on the DB with the new data
                    ...
                    //update ordered_items on DB with the new data (one record for each row of the table)
    ...
    private void checkState(String state, CustOrder cor) {
        if (state.equals("View")) {
            System.out.println("View State");
            txtShip_no.setEditable(false);
            datepicker.setEditable(false);
            comboCustId.setEditable(false);
            txtCustName.setEditable(false);
            txtProductName.setVisible(false);
            txtDescription.setVisible(false);
            txtUnitCost.setVisible(false);
            ...
            stage.setTitle("Edit Customer Order");
        }
    }


    public void loadForm(CustOrder cor) {
        txtShip_no.setText(cor.getNumber());
        datepicker.setValue(cor.getDate());
        comboCustId.setValue(cor.getCustomerId());
        txtCustName.setText(cor.getCustomerName());
    }


    public void loadTableView(int ship_no) {
        Statement stmt = null;
        CustOrder cor = new CustOrder();
        try {
            CallableStatement callstmt = connection.prepareCall("call loadCusItems(?)");
            callstmt.setInt(1, ship_no);

            ResultSet newresult = callstmt.executeQuery();

            while(newresult.next()){
                cusItemsList.add(new OrdItems(
                        newresult.getString("productName"),
                        newresult.getString("productDescription"),
                        newresult.getDouble("unitCost"),
                        newresult.getDouble("tax"),
                        newresult.getInt("discount"),
                        newresult.getInt("pieces"),
                        newresult.getDouble("finalCost")));
                tableView.setItems(cusItemsList);
            }
            newresult.close();
            callstmt.close();
        } catch (SQLException d) {
    ...
    public void updateStorage(){
        ...
        //for each row of the tableView update storage
        for (int i = 0; i < tableView.getItems().size(); i++) {
            try {
                String updQuery = "UPDATE storage_area" +
                        " SET pieces = pieces - ? " +
    ...
    public boolean checkAvailability() {
        ...
        //search in the storage area and find the values of the items listing in the tableview (current storage values)
        try {
            Statement stmt = connection.createStatement();
            ResultSet resultSet = stmt.executeQuery("SELECT DISTINCT r.productname, p.productdescription, s.pieces, s.sup_id \n" +
            ...
            while (resultSet.next()) {
                ...
                System.out.println("=========================");
                mapStorage.put(st.getProductName(), st.getItems());
            }
        ...
        //search into the ordered_items and find the values of the items of the shipping_order (before editing)
        try {
            Statement stmt = connection.createStatement();
            ResultSet resultSet = stmt.executeQuery("SELECT DISTINCT r.productname, p.productdescription, r.pieces, p.sup_id \n" +
                    "         FROM ordered_items r JOIN products p  INNER JOIN \n" +
            ...
            while (resultSet.next()) {
                ...
                System.out.println("=========================");
                mapOrdered.put(st.getProductName(), st.getItems());
        ...
        //search into the tableView_products_temp and find the values of the items of the current tableview (current values-after editing)
        try {
            Statement stmt = connection.createStatement();
            ResultSet resultSet = stmt.executeQuery("SELECT DISTINCT r.productname, p.productdescription, r.pieces, p.sup_id \n" +
            ...
            while (resultSet.next()) {
                ...
                System.out.println("=========================");
                mapTableView.put(st.getProductName(), st.getItems());
        ...
        System.out.println("==================");
        System.out.println("mapOrdered.size(before): " + mapOrdered.size());
        System.out.println("==================");
        System.out.println("mapTableView.size(before): " + mapTableView.size());

        //make sure that every key in one map corresponds to a key in another map (mapTableView, mapOrdered)
        Iterator<String> keys = mapTableView.keySet().iterator();

        while (keys.hasNext()) {
            String key = keys.next();
            if (!mapOrdered.containsKey(key)) {
                mapOrdered.put(key, 0);
                System.out.println("key "+ key +" was added to mapOrdered");
        }

        }
        System.out.println("==================");
        System.out.println("mapOrdered.size: " + mapOrdered.size());

        Iterator<String> keys2 = mapOrdered.keySet().iterator();

        while (keys2.hasNext()) {
            String key2 = keys2.next();
            if (!mapTableView.containsKey(key2)) {
                keys2.remove();
                System.out.println("key "+ key2 +" was removed from mapOrdered");
            }
        ...
        System.out.println("mapOrdered.size: " + mapOrdered.size());
        System.out.println("==================");
        System.out.println("mapTableView.size: " + mapTableView.size());

        //subtract values of mapOrdered from values of mapTableView in order to correct storage values
        Set<String> keySet = new HashSet<String>();
        keySet.addAll(mapTableView.keySet());
        keySet.addAll(mapOrdered.keySet());
        keySet.addAll(mapStorage.keySet());


        for (String key : keySet) {
            mapSubtraction.put(key, mapTableView.get(key) - mapOrdered.get(key));
        }

        System.out.println("Subtraction map: " + mapSubtraction.toString());
            ...
            for (String key : keySet) {
               ...
               if(mapStorage.get(key) < mapSubtraction.get(key)){
                   exceeds.append("Product ").append(key).append(": You can require up to ").append(mapStorage.get(key) + mapOrdered.get(key)).append(" items in total.\n");
               }
            }
            System.out.println("exceeds: " + exceeds);
